...
function LinkCard({ link, index = 0, featured = false }) {
  const getIcon = () => {
    switch (link.icon) {
      case 'instagram':
        return (
  ...
  return (
    <motion.a
      ...
      className={`group relative block overflow-hidden rounded-lg border border-white/10 bg-gradient-to-r ${link.color} p-[1px] transition-all hover:scale-[1.02]`}
      ...
      whileTap={{ scale: 0.98 }}
    >
      <div className={`flex items-center justify-between rounded-lg bg-black p-4 ${featured ? 'md:p-6' : ''}`}>
        <div className="flex items-center gap-4">
          {/* Icon */}
          <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg bg-white/10 text-white">
            {getIcon()}
          </div>
        ...
        {/* Arrow */}
        <div className="flex-shrink-0 text-white/40 transition-all group-hover:translate-x-1 group-hover:text-white">
          <svg
            className="h-5 w-5"
            fill="none"
            viewBox="0 0 24 24"
